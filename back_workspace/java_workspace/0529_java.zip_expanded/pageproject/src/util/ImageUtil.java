...
//어떤 프로그램이던 이미지와 관련된 유용한 기능을 이 클래스에서 제공해 줌
public class ImageUtil {
	...
	public Icon getIcon(String filename, int width, int height) {
		...
		//패키지 안에 들어있는 이미지 자원의 이름을 명시하면, url을 반환해줌.
		URL url = myClass.getClassLoader().getResource(filename);
		
		//ImageIcon 자체는 크기를 지정할 수 없으므로, 아이콘 사용에 앞서 크기 조절이 가능한 
		//이미지를 먼저 얻어야 함.
		//Toolkit을 통해 Image 객체는 픽셀 정보까지 접근할 수 없기 때문에 단순 이미지를 보여줄 때 이용가치가 있다.
		//회사의 이미지 로고를 박은 워터마크 처리 등의 보다 섬세하고 기술적인 이미지 제어를 하려면 BufferedImage를 사용
		ImageIcon icon=null;
