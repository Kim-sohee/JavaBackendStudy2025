...
//Redis 관련 로직: 임시 가입 정보를 저장하고, 검증
@Slf4j
@Service
public class RegistRedisService {
    //redis에게 데이터를 넣을때 문자열화 시켜서 넣어야 하므로, 그 문자열 작업을 쉽게 처리해주는 라이브러리
    ...
    //키설계(redis에 사용될 키 규칙 - 개발자가 정의)
    private String pendingKey(String email){
        return "pending:" + email;      //pending: sohee@naver.com
    }
    //이메일을 조회하기 위한 키 설계
    ...
    public void savePending(MemberDTO memberDTO){
        //Member라는 java 객체가 redis로 insert 되려면, 문자열화 되어야 한다.
        try {
