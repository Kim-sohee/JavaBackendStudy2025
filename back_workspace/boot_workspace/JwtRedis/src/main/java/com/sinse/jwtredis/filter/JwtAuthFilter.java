...
* 1) 클라이언트의 헤더에 들어있는 Authrozation 헤더에서 Bearer 토큰 추출
* 2) JWT 서명 조작여부/ 만료 기간 검증 (유효한 토큰 인지 판단)
* 3) 블랙리스트 여부 검증(이미 로그아웃한 유저인 경우 401로 전송)
* 4) 문제 없으면, 시큐리티에게 인증을 알려야 한다.
**/
@Slf4j
@Component
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        * ----------------------------------------------------*/
           ...
           if (header != null && header.startsWith("Bearer ")) {
            ...
            /*-----------------------------------------------------
                2) JWT 파싱 및 서명 조작 여부, 기간 만료 등 검증
            * ----------------------------------------------------*/
               Jws<Claims> jws=jwtUtil.parseToken(token);
               Claims claims=jws.getBody(); //jjwt 11.xxx , 12이상은 getPayLoad()
               
           /*-----------------------------------------------------
                3) Claims로 부터 필요한 값을 추출하여, 블랙리스트 여부 판단
                    - Jti: UUID 고유값
                    - sub: 로그인사용자
                    - ver: 디바이스에 대한 전역적 토큰 버전
            * ----------------------------------------------------*/
               ...
               //아래의 코드를 처리해야만, SecurityConfig의 anyRequest().authenticated()를 무사히 통과
               /*
               * [ details와 principal의 차이 ]
               * principal : 누가 로그인했는가? (보통 UserDetails 또는 User)
               * credential : 어떻게 로그인 했는가? (비밀번호, JWT 토큰 등)
           ...
           //기타 예외
           response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
           response.getWriter().write("{\"error\": \"인증 에러\"}");
       }
    }
