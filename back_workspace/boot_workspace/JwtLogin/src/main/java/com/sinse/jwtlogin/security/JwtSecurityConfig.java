...
public class JwtSecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http, OncePerRequestFilter jwtAuthFilter, DaoAuthenticationProvider provider) throws Exception {
        http
                ...
                //세션을 만들지도, 사용하지도 않도록 정책을 STATELESS로 변경해야 함
                //시큐리티가 인증 상태를 세션에 더 이상 자동으로 저장하지 않게 한다.
                .sessionManagement(session->session.sessionCreationPolicy(SessionCreationPolicy.STATELESS))

                //SecurityContext 저장소를 null 저장소로 교체, 즉 인증 결과를 세션/서버 상태에 저장하지 않음(완전 무상태를 보장하기 위함)
                .securityContext(sc->sc.securityContextRepository(new NullSecurityContextRepository()))

                //시큐리티의 폼로그인 기능을 이용하면, 컨트롤러 없이도 로그인 인증이 처리되지만, JWT에서는 개발자가 토큰을 컨트롤러에서 발급해야 하므로,
                //컨트롤러를 활성화 시키고, 대신 formLogin 기본 기능을 비활성화
