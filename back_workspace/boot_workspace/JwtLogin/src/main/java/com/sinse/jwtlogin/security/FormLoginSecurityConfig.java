...
//스프링 시큐리티를 개발자가 원하는 설정으로 변경하고자 할 때 이 클래스 정의가 필요함
//@Configuration
//@EnableWebSecurity
public class FormLoginSecurityConfig {
    ...
    public PasswordEncoder passwordEncoder(){
        return new BCryptPasswordEncoder();     //비번 결과물 자체에 salt가 포함됨
    }
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        return http
                //스프링시큐리티가 기본적으로 CRSF 방지기능이 있음, 아래의 코드로 disable 시키면 CSRF 비활성화
                ...
                .formLogin(form -> form
                        ...
                        //동기 방식으로 요청이 들어올때, 결과를 html 보여줄때 아래의 코드를 사용
                        //.defaultSuccessUrl("/index.html")       //로그인 성공 후 보여질 url

                        //비동기 방식의 요청이 들어올때는, json으로 결과를 보여줘야 하므로, 컨트롤러를 작성하지 않고도 json 결과를 전송할 수 있다.
                        .successHandler((request, response, auth)->{
                            ...
                            */
                            Map map = Map.of("result", "로그인성공", "user", auth.getName());   //값 고정이므로, 생성 후 변경 불가
