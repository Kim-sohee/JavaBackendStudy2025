...
public class JpaBoardDAO implements BoardDAO {
    //Hibernate의 경우 session 객체가 쿼리 수행 객체이지만
    //JPA에서는 EntityManager를 쓴다. 최상위 인터페이스이므로, 하이버네이트를 사용하 경우에도
    //session 대신에 EntityManager를 쓸 수 있다.
    ...
    public void delete(int board_id) {
        //삭제 시 무조건 일치하는 pk를 지우지 말고, 실제적으로 존재하는 데이터인지 먼저 체크한 후 삭제하는 게 안전
        Board board = em.find(Board.class, board_id);
        if(board!=null){    //데이터가 존재한다면
            em.remove(board);
        }
    }
}
