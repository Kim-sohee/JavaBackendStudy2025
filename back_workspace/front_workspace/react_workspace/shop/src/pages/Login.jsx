...
export default function Login() {
  ...
  const loginRequest = (provider) => {
    //원래는 각 provider 마다 인증 url이 틀리므로, 조건으로 처리해야 하지만,
    //spring boot에서 지원하는 oauth2 라이브러리를 사용중이므로, 이 url을 서버측 스프링이 알아서 처리해준다.
    ...
    <div className="container mt-5 col-10 col-md-6">
      ...
      <form className="p-4 border rounded bg-white shadow-sm" onSubmit={handleSubmit}>
        ...
        <div className="d-grid gap-2">
          <button type="button" className="btn btn-success" onClick={()=>{loginRequest('naver')}}>Naver</button>
          <button type="button" className="btn btn-warning text-dark" onClick={()=>{loginRequest('kakao')}}>Kakao</button>
          <button type="button" className="btn btn-danger" onClick={()=>{loginRequest('google')}}>Google</button>
        </div>
