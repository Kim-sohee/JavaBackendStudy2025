...
public class DispatcherServlet extends HttpServlet{
	...
	//이 서블릿의 인스턴스가 생성될 때 초기화가 진행되고, 이 초기화 지행 시점에 설정 파일로 부터, 해당 요청에 어떤 하위 컨트롤러가 작동해야 하는지 분석하여
	//요렁을 전달하기 위해
	FileInputStream fis;
	Properties props;		//java.util.Map의 자식 중 Properties 라는 객체는 자신이 읽얻르인 문자열이 만일 =을 기준으로 key=value 형태로 되어 있다면,
								//이것을 해석할 수 있는 객체이다.
	
	public void init(ServletConfig config) throws ServletException {
		ServletContext context = config.getServletContext(); //현재 서블릿이 실행되고 있는 문맥상의 애플리케이션
		
		//WEB-INF: 외부 웹 브라우저에서 접근이 불가능한 보안 디렉토리이지만, 자바 코드 내부에서는 얼마든지 접근 가능
