...
//post로 전송되어 오는 로그인 정보를 받아, 회원 테이블과 일치 여부에 따라 로그인 성공 및 실패
public class LoginServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		logger.debug("넘겨 받은 아이디 " + id);
		logger.debug("넘겨 받은 비밀번호 " + pwd);
		...
		try {
			...
			//로그인이 성공되면 세션 객체에 담자
			//jsp에서는 내장객체로서, session이 지원되지만 현재 이 코드는 서블릿이기 때문에 개발자가 자료형으로 제어해야 함
