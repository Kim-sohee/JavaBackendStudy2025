...
public class CharacterEncodingFilter implements Filter{
	...
	//서블릿의 생명주기와 완전 흡사, 아래의 init()은 필터 인스턴스가 생성되자마자 호출되는 필터 초기화 메서드이다.
	//개발자는 web.xml에 초기값을 전달하려면 이 메서드에서 진행할 수 있다.
	@Override
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		//chain.doFilter()를 기준으로 이전에 코드를 작성하면, 요청이 들어올 때 하고 싶은 작업을 처리...
		chain.doFilter(request, response);		//다음 필터 또는 다음 서블릿으로 요청 전달
		//chain.doFilter()를 기준으로 다음에 코드를 작성하면, 응답 시 하고싶은 작업을 처리...
