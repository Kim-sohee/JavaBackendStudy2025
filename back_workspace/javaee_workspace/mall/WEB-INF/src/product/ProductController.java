...
/*아래의 클래스는 오직 웹 애플리케이션 서버에서만 해석 및 실행될 수 있는 javaEE 기반의 클래스 정의
...
public class ProductController extends HttpServlet{
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
		//response 객체로 부터 스트림을 얻기 전에 한글 인코딩 지정
		response.setContentType("text/html");
		response.setCharacterEncoding("utf-8");
		
		//클라이언트의 웹 브라우저에 출력될 문자열을 스트림으로 준비해보자.
		PrintWriter out = response.getWriter();
		out.print("Kim So Hee");
		...
		try{
			...
			if(con != null){
				...
				rs = pstmt.executeQuery();		//select 수행 및 표 반환Product.java
		...
		//List에 들어 있는 객체들을 html table로 출력해보기
