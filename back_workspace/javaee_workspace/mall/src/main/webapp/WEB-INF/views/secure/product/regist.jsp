              ...
              <form id="form1">
                <div class="card-body">
                    ...
                    <div class="col-sm-6">
                      <div class="form-group">
                        <label>하위 카테고리</label>
                        <select class="form-control" id="subcategory" name="subcategory.subcategory_id">
                        </select>
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="form-group">
                   ...
                   <div class="form-group">
                     ...
                     <div class="form-group">
	                    <input type="text" class="form-control" id="product_name" placeholder="상품명 입력">
	                  </div>
...
<script>
	...
	function getTopCategory(){
		$.ajax({
			...
			success: function(result, status, xhr){	//200번 대의 성공 응답 시, 이 함수 실행
				console.log("서버로 부터 받은 결과는 ", result);
				//화면에 출력하기
	...
	//HTML 작성된 기존 폼에서 텍스트 입력 관련된 컴포넌트는 사용하되, 이미지 업로드 컴포넌트는 재설정해야 한다.
	let selectedFile = [];

    function regist(){
		//기존 폼을 이용하되, file 컴포넌트 파라미터만 새로 교체(selectedFile 배열로 대체)
		//js에서 프로그래밍 적 form을 생성
		let formData = new FormData(document.getElementById("form1"));
		
		//formData는 동기/비동기 둘다 지원하지만, 대부분은 비동기방식을 많이 쓴다.
		//Jquery Ajax 자체에서 formData를 비동기 방식으로 간단하게 사용할 수 있는 코드를 지원
		//기존 photo 버리고, 우리가 선언한 배열로 대체
		//formData.append("email", "sohee1234@naver.com");	//<input type="text" name="email">과 동일
		//formData는 개발자가 명시하지 않아도, 디폴트로 multipart/form-data가 지정되어 있다.
