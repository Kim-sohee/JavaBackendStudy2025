...
public class NoticeController {
	//규모가 작은 프로젝트에서는 상관없으나, 규모가 엄청 큰 규모의 경우에는 유지보수 시간이 곧 비용이므로,
	//최대한 유지보수성을 높이려면 객체와 객체들간의 has a 관계는 느슨할수록 좋다.(상위자료형으로 보유할 수록 느슨해진다..)
	...
	public ModelAndView selectAll() {
		...
		mav.setViewName("secure/notice/list"); 	//이것만 넘기는 DispatcherServlet, ViewResolver가 해석
		
		return mav;
	}
	//상세보기 요청 처리
	//글쓰기 폼 요청 처리(write.jsp가 WEB/-INF 안에 위치해있기 때문에, 하위 컨트롤러에 의해서만 접근 가능, 외부 브라우저에서 주소값으로 직접 접근 불가능)
	...
	public ModelAndView regist(Notice notice) {
		...
		return mav;		//클라이언트로 하여금 지정된 url로 재접속
	}
